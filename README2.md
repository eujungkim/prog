...
- Dataset : Training, Validation, Test (8:1:1, 6:2:2)
  ...
  - Test set : 학습에 관여하지 않고, 모델의 최종 성능을 평가하기 위한 데이터. 여러 모델 간 성능을 비교할 때 Test set에 대한 스코어를 활용한다.
...
- AutoML 서비스
  ...
  - Google AutoML
    ...
    - 몇 시간~ 1일 이내의 학습 시간, 고비용, 예산 커트라인 지정하여 해당하는 만큼만 학습 가능
    - Google AutoAM로 학습된 모델은 어떤 하이퍼파라미터를 활용하며, 어떤 네트워크 아키텍처를 가지고 있는지 알 수 없음
    - 비용을 지불하고 해당 데이터에 대해 최적으로 맞춰진 모델을 이용할 수 있을 뿐
## 12편
XAI
...
- 1) 기존 AI 모델에 설명할 수 있는 어떤 모듈을 덧붙이는 방식
  ...
  - 설명하는 법 학습하기(Learn to explain) : 딥러닝 모델에 RNN 모듈 등을 덧붙여 인간이 이해할 수 있는 방식으로 설명을 생성하도록 하는 방식 (설명 생성에 한계가 있어 널리 활용되지는 않음)
...
- 3) 인공신경망처럼 복잡한 블랙박스 모델의 일부분을 설명해 줄 수 있는 다른 모델을 활용하여 유추하는 방식
  - 일부 영역의 데이터(+와 O)를 활용해 설명력이 좋은 모델(회색 점선)을 별도로 하나 더 만들어 학습시킨다
  - ![image](https://github.com/eujungkim/prog/assets/7552395/e75dd1b5-8b50-42df-8b06-47b1066ccad3)
  - LIME, SP-LIME
